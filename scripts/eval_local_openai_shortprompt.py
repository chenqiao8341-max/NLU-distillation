#!/usr/bin/env python3
"""
Evaluate local OpenAI-compatible NLU models with the same short prompt used by SFT.

The previous local evaluator injected long prompt.md sections as system messages.
This runner keeps evaluation inside this repository and uses the current SFT
instruction from data/processed/<task>/sft/all.jsonl instead:

  user = instruction + "\n" + query

That matches LLaMA-Factory's Alpaca conversion for the current training data.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="短 prompt 本地 OpenAI-compatible NLU 评测")
...
def load_short_instruction(task_type: str) -> str:
    slug = TASK_SLUGS[task_type]
    path = ROOT / "data" / "processed" / slug / "sft" / "all.jsonl"
...
def evaluate_task(
    ...
    instruction = load_short_instruction(task_type)
    total = len(dataset)
...
def write_markdown(path: Path, payload: dict[str, Any]) -> None:
    lines = [
        f"# {payload['model']} short-prompt local NLU evaluation",
        "",
...
def main() -> None:
    ...
    out_prefix = Path(args.output_prefix) if args.output_prefix else ROOT / "reports" / "shortprompt" / f"local_{safe_model}_{timestamp}"
